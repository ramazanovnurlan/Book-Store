...
export class LanguageListComponent implements OnInit {
  ...
  returnAgain(){
    this.optionService.getLanguageList(this.langService.getLang().toUpperCase()).subscribe(data => {
...
goto_edit(id:number){
  ...
  this.router.navigate(["/cabinet/language-edit/"+id]);
}

deleteLanguage(id:number){
  this.optionService.deleteLanguage(id,this.langService.getLang().toUpperCase()).subscribe(data => {
    if (data.errorCode && data.errorCode == 1) {
...
clickCreate(){
  this.router.navigate(["/cabinet/new-language"]);
}
