...
export class AuthorListComponent implements OnInit {
  ...
  displayedColumns: string[] = ['id','fullName','biography','op','actions'];
  ...
  returnAgain(){
      this.optionService.getAuthorList(this.langService.getLang().toUpperCase()).subscribe(data => {
  ...
  goto_edit(id:number){
    ...
    this.router.navigate(["/cabinet/author-edit/"+id]);
  }
  deleteAuthor(id:number){
    this.optionService.deleteAuthor(id,this.langService.getLang().toUpperCase()).subscribe(data => {
      if (data.errorCode && data.errorCode == 1) {
  ...
  clickCreate(){
    this.router.navigate(["/cabinet/new-author"]);
  }
